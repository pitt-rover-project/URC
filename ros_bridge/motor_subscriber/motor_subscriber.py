...
class MotorSubscriber(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info(
                "Serial connection established on /dev/ttyACM1 at 115200 baud"
            )
            ...
            self.get_logger().error(
                f"Failed to connect to serial port /dev/ttyACM1: {e}"
            )
            self.serial = None
        ...
        # Create timer to periodically read from Arduino to prevent buffer overflow
        # This drains the receive buffer so Arduino acknowledgments don't cause blocking
        self.create_timer(0.05, self.read_serial_buffer)  # Read every 50ms
    ...
    def read_serial_buffer(self):
        """Periodically read from serial to prevent buffer overflow"""
        if self.serial is None or not self.serial.is_open:
            return

        try:
            # Read all available data from the buffer without blocking
            ...
            self.get_logger().error(f"Error reading serial buffer: {e}")
    ...
    def twist_callback(self, msg):
        ...
        if self.serial is None:
            self.get_logger().warning(
                "Cannot send Twist command: serial connection not available"
            )
            return
