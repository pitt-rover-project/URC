...
from publishers.publisher import MotorPublisher
import rclpy
...
class MainWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.ros_status = QLabel(
            "ROS2: Connected" if self.motor_publisher else "ROS2: Disconnected"
        )
    ...
    def update_physics(self):
        ...
        if self.shift_pressed and (
            Qt.Key_W in self.keys_pressed
            or Qt.Key_S in self.keys_pressed
            or Qt.Key_A in self.keys_pressed
            or Qt.Key_D in self.keys_pressed
        ):
            ...
            if Qt.Key_W in self.keys_pressed:
                ...
                self.linear_velocity = min(
                    target_linear, self.linear_velocity + self.acceleration_rate
                )
            ...
            if abs(self.linear_velocity) > 0.001:
                if self.linear_velocity > 0:
                    self.linear_velocity = max(
                        0, self.linear_velocity - self.deceleration_rate
                    )
                else:
                    self.linear_velocity = min(
                        0, self.linear_velocity + self.deceleration_rate
        ...
        self.throttle_status.setText(
            f"Throttle: {'ON' if self.shift_pressed else 'OFF'}"
        )
    ...
    def display_values(self):
        ...
        print(
            f"Linear: {self.linear_velocity:.3f} | Angular: {self.angular_velocity:.3f} | Throttle: {self.throttle_control.get_throttle_value():.2f}"
        )
...
if __name__ == "__main__":
    ...
    motor_pub = MotorPublisher(
        topic_name="motor_control_input", node_name="rover_control_gui"
    )
